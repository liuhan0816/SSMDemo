    ...
    Class.prototype.tree = function(elem, children){
        ...
        layui.each(data, function(index, item){
            var hasChild = item.children && item.children.length > 0
                ...
                ,function(){
                    if(options.showLine){
                        var a='<span class="layui-tree-iconClick layui-tree-icon"><i class="layui-icon '+ (item.spread ? "layui-icon-subtraction" : "layui-icon-addition") +'"></i></span>';
                        if(item.menuIcon){
                            a=a+'<span class="layui-icon ';
                            a=a+item.menuIcon;
                            a=a+'"></span>';
                        }
                        return a;
